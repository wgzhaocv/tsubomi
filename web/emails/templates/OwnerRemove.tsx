...
export default function OwnerRemove() {
  return (
    <Layout preview="tsubomi の 管理者権限が解除されました">
      <Badge color={C.danger}>管理者権限の変更</Badge>
      <Text style={heading}>管理者権限が解除されました</Text>
      <Text style={paragraph}>
        あなたの tsubomi の <strong>管理者権限</strong> が解除されました。
        これ以降、管理画面の操作(他ユーザの資源の停止 / 削除、共有パスワード、IP
        許可リスト、管理者管理)は行えません。
      </Text>
      <Text style={{ ...paragraph, color: C.muted, fontSize: "13px", margin: 0 }}>
        必要であれば、別の管理者にあらためて追加を依頼してください。心当たりがない場合は
        別の管理者に確認してください。
      </Text>
    </Layout>
  );
