...
| 4 | registry と deploy hook は**会社 IP 許可リストから除外**(公開 + 各自の認証) | GitHub-hosted runner の IP は予測不能で、インターネットからの到達が必須;認証は htpasswd / HMAC |
...
- ユーザ service:`<service[-ランダム語]>.<ドメイン>`(会社 CIDR。公開範囲は後に **visibility 三態**
  private/company/public に発展 — `paas-service-visibility-design.md`)
...
create table service_details (
  ...
  -- 当初ここに在った public bool は一度も配線されず drop(20260622)。後に visibility text 三態
  -- (private/company/public、20260702)として実現 — paas-service-visibility-design.md
...
(この表は設計時の計画面。以後のフェーズ / マイルストーン外追加で入った面 — rollback / exec /
terminal / env resolved / deploy-config / **visibility** / cache 一式 / db public の開閉 /
registry 直連など — は各フェーズの design doc と CLAUDE.md が正)
...
| **M3 service** ✅ | registry 開始 + create(ユーザ gh オーケストレーション + workflow テンプレート)→ Action → hook(HMAC/nonce/digest)→ コンテナ + traefik ルーティング(**file provider**)+ limits + logs/start/stop/rollback + **注入機構**(db/volume → service;静的 env)+ reconcile v1(存在 + 孤児)。**実装級の詳細は `paas-m3-design.md`(第 5 層)**。prod-infra 込みで完了(`tsubomi-app.com` で本番稼働・両デプロイ経路の端到端検証済み) | push から 30 秒で `https://myapp.<ドメイン>` が開く;`tbm inject` で app が DB / ボリュームに繋がる;ホスト再起動から自己回復(本番 end-to-end 済み) |
| **M4 ガバナンス** ✅ | 管理画面(owner-gated の総覧 / ランキング / 最後の砦の操作 + Resend 検証コード)+ ディスク警報 + audit_log 閲覧。**実装級は `paas-m4-design.md`**(S1 可視化 / S2 メール+ディスク警告 / S3 最後の砦 / S4 audit 閲覧)。共有パスワード viewer は否決・後相(§10-A)| owner が見える・対処できる(完了・dev e2e 済み) |
| **M5 cache(valkey)** ✅ | ACL:namespace プレフィックス + コマンド許可リスト;REDIS_URL 注入。**実装級は `paas-m5-design.md`** | 越境は NOPERM(完了・本番稼働) |
| **M6** | compose 複数コンテナ service(cgroup parent slice で合算クォータ)+ passkey + クォータ数値の調整 | |

マイルストーン後の追加(いずれも実装・本番稼働済み。現況の正は CLAUDE.md、実装級は各 design doc):
service↔service 内部リンク(`paas-service-link-design.md`)/ コンテナ内アクセス terminal・exec
(`paas-terminal-design.md`)/ **service 公開範囲 visibility**(`paas-service-visibility-design.md`)/
公開 DB・公開 cache(`paas-db-public-design.md`・`paas-cache-public-design.md`)/ registry 直連入口
(`paas-registry-direct-design.md`)/ egress 出站フィルタ(`paas-egress-design.md`)。
...
- **registry + hook がインターネットから到達可能**(決定 #4):認証は htpasswd / HMAC だけで
