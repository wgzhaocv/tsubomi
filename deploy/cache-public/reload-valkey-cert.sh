#!/usr/bin/env sh
# acme.sh の reloadcmd から呼ぶ:cache.tsubomi-app.com の LE 証書を valkey の TLS ボリュームへ入れ、
...
#   CACHE_TLS_VOL valkey TLS ボリューム名(既定 tsubomi-deploy_cache_tls = compose の cache_tls)
...
# 1) LE 証書を cache_tls ボリュームへ(一時 alpine コンテナで cp)。valkey は読み取り専用 mount なので
...
# valkey が未起動(初回ブートストラップ等)なら CONFIG SET は通らないが、証書は既にボリュームへ配置済みなので
...
if docker exec tsubomi-valkey valkey-cli --user tsubomi-admin -a "$PASS" --no-auth-warning PING >/dev/null 2>&1; then
  ...
  echo "valkey 未起動 — 証書はボリュームに配置済み。次回起動で反映されます。"
fi
