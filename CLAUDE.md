...
M0 基盤(ログイン/CLI token)→ M1 database → M2 volume → M3 service(デプロイ
経路+注入)→ M4 ガバナンス → M5 valkey。各フェーズ単体で使える状態にする。
マイグレーションはフェーズ毎に追加。
...
- `tbm login` は **RFC 8252 loopback**(127.0.0.1 の一回限りリスナー)が
  デフォルト:ブラウザで「許可する」を押すだけでコピペ不要。`--manual` で
  コピペ方式(SSH 先などブラウザと CLI が別マシンの場合)。サーバ側の
...
```bash
just db-up        # 管制面 postgres 起動(127.0.0.1:5434。マイグレーションはサーバ起動時に自動)
