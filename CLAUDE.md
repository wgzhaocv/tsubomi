...
graceful stop。**副産物の発見 → 同日修正(v41)**:registry GC の keep-set 欠陥(tag 再利用で直近成功
digest が `--delete-untagged` に回収され start/rollback が pull 404 — 既存バグ)。修正 =
`registry::protect_and_expire_manifests`(日次 GC 前段):現役 ∪ 直近 5 成功版に保護 tag
`keep-<digest12>` + terminal 旧版の明示 DELETE(rollback 実効窓 = 5 版に確定)。**deploy 成功の
瞬間にも keep tag を付与**(併行 push 窓を閉じる — codex)。実装級・地雷・受容は
**`doc/paas-service-stateful-design.md` §10-E**。
