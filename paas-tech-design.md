...
| **M3 service** | registry 開始 + create(ユーザ gh オーケストレーション + workflow テンプレート)→ Action → hook(HMAC/nonce/digest)→ コンテナ + traefik ルーティング + limits + logs/start/stop + **注入機構**(db/volume → service;静的 env)+ reconcile v1(存在 + 孤児)。**実装級の詳細は `paas-m3-design.md`(第 5 層)** | push から 30 秒で `https://myapp.<ドメイン>` が開く;`tbm inject` で app が DB / ボリュームに繋がる;ホスト再起動から自己回復 |
