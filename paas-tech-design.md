...
コンテナ env = service_env(静的、復号)
            ∪ injections を 1 件ずつ解決:
                database → DATABASE_URL = postgres://<role>:<pass>@db.<ドメイン>:6432/<dbname>
                cache    → REDIS_URL    = redis://:<pass>@cache.<ドメイン>:6379  (M5)
                volume   → host_path を mount_path にマウント;env_var = mount_path;
                           ディレクトリが無ければ先に mkdir
```

- 接続文字列は **pgbouncer の外部ドメイン**を指す。コンテナ内でも社内デバッグでも
  同じ 1 本(§1 ネットワーク要点)。
...
- 未決(いずれも M0–M2 を塞がない):pgbouncer の TLS;compose_spec のスキーマ
