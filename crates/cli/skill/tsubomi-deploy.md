...
補足(tbm 1.0.23+):**サービスが複数でも service の repo 内なら `--service` 不要**(repo の
...
コンテナが起動即 exit した失敗デプロイは、**エラーに終了要因(exit code / OOM。server v44+)と
ログ末尾(以前から)が載る**。`tbm service status <名前>` / `verify --wait` の error を**まず読む**:
