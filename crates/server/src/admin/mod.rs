...
//! 最後の砦(動かす)の 2 枚。**owner 機能は web 専用** — 各ハンドラは owner 身分 **かつ**
//! session 由来を毎回検証する(Bearer cli_token では触れない。CLI は AI 駆動のユーザ
//! リソース操作専用というプロジェクト規約)。フロントエンドの表示制御はただの UX。
//!
//! S1 可視化(overview / ranking):ユーザ横断の匿名化された一覧。真名は出すが、リソースは
...
pub fn routes() -> Router<AppState> {
    Router::new()
        ...
        // アップグレードは cookie 付き GET なので require_auth + AuthCtx が効く(handler 内で viewer 検証)。
        .route("/admin/metrics", get(crate::metrics::metrics_ws))
        // 最後の砦(S3):owner が他人のリソースを停止 / 削除(二段確認 + 検証コード)。
...
/// 設計 v2 §7「見るは共有密码」— 共有パスワードを入れた社内ユーザは読み取り専用で管制面を
/// 見られる。**読み取り専用の可視化(overview / ranking)専用**。危険操作(stop/delete)・
