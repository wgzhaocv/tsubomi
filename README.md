...
REGISTRY=docker.io/wgzhaofumi IMAGE=tsubomi TAG=v4 just release-image  # multi-arch push
# just 無し:  REGISTRY=docker.io/wgzhaofumi IMAGE=tsubomi TAG=v4 bash scripts/release-image.sh
```

publish 後、VPS 側は新タグで起こし直すだけ:
`TSUBOMI_IMAGE=docker.io/wgzhaofumi/tsubomi:v4 docker compose --env-file .env.production -f compose.prod.yml up -d`
